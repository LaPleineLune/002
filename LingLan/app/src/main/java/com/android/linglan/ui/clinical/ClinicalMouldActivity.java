 ...
 * 病情模板
 */
public class ClinicalMouldActivity extends BaseActivity implements AlertDialoginter {
...
//        for(ClinicalDetailsBean.ClinicalDetailsData.ClinicalDetailsPathography clinicalMouldData : pathography){
//            LogUtil.e(" 123123123clinicalMouldData.templetname == " + clinicalMouldData.templetname);
//        }
...
//                                        LogUtil.e("没有勾选掉的id号msClinicalMouldBean.data.get(j).templetid" + id);
                        ...
                        if (mClinicalMouldBean != null) {
                            alertflag = 1;
                            deleteCount.clear();
                            StringBuilder name = new StringBuilder();
                            for (int i = 0; i < msClinicalMouldBean.length; i++) {
                                if (!msClinicalMouldBean[i].equals("清空") && SharedPreferencesUtil.getString(msClinicalMouldBean[i], null) != null) {
//                                    count = 1;
                                    deleteCount.add(i);
                                    name.append(" " +
                                            "[");
    ...
    public void getClinicalAllMould() {
        NetApi.getClinicalAllMould(new PasserbyClient.HttpCallback() {
            ...
            public void onSuccess(String result) {
                ...
                if (!HttpCodeJugementUtil.HttpCodeJugementUtil(result, ClinicalMouldActivity.this)) {
                    return;
                }
...
//                    LogUtil.e("清掉的内容更msClinicalMouldBean.data.get(deleteCount.get(i)).templetname = " + msClinicalMouldBean[deleteCount.get(i)] + "deleteCount.get(i) = " + deleteCount.get(i));
        ...
        }, templet);// templet   {"templetname":"主诉,现病史,诊断"}
    }
