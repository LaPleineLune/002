package com.android.linglan.manager;
...
public class ThemeManager {
    ...
    public int getCurrentColor() {
        return SharedPreferencesUtil.getInt(KEY, BACKGROUNDS[0]);
    }

    public void saveColor(int index) {
        SharedPreferencesUtil.saveInt(KEY, BACKGROUNDS[index]);
        notifyThemeChange();
    }

    public void registerListener(IThemeListener listener) {
        if (iThemeListeners == null) {
            iThemeListeners = new ArrayList<IThemeListener>();
        }
