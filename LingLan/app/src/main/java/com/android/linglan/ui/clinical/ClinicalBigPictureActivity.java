...
import com.android.linglan.camerautils.AlbumViewPager;
import com.android.linglan.camerautils.FileOperateUtil;
import com.android.linglan.camerautils.MatrixImageView;
import com.android.linglan.camerautils.VideoPlayerContainer;
 ...
 */
public class ClinicalBigPictureActivity extends Activity implements OnClickListener, MatrixImageView.OnSingleTapListener
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_clinical_big_picture);
        ...
        updataPhotoBeens = (List<UpdataPhotoBeen>) getIntent().getSerializableExtra("updataPhotoBeens");
        currentItem = getIntent().getIntExtra("position", 0);

        if (updataPhotoBeens.size() > 0) {
            List<String> paths = new ArrayList<String>();
//            int currentItem = 0;

            for (UpdataPhotoBeen updataPhotoBeen : updataPhotoBeens) {
                if (updataPhotoBeen != null && !updataPhotoBeen.data.mediaurl.equals(""))
//                    currentItem = updataPhotoBeens.indexOf(updataPhotoBeen);
                paths.add(updataPhotoBeen.data.mediaurl);
                LogUtil.e("pathpathpathpathpath" + updataPhotoBeen.data.mediaurl);
            }
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            case R.id.header_bar_photo_back:
                startActivity(new Intent(this, ClinicalBigPictureActivity.class));
                break;
